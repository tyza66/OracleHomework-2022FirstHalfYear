	...
	<body>
		<% 
			...
			<div id="app">
				...
				<el-main class="main">
					<el-menu default-active="7" class="el-menu-vertical-demo lefttab">
						...
						<el-menu-item index="3" @click="gopage(3)"> <i class="el-icon-setting"></i> <span
								slot="title">活动管理</span> </el-menu-item>
					...
					<div class="righttab">
						<div style="margin: 10px 0;">
							...
							<el-button type="primary" @click="reload()">取消查询</el-button>
						</div>
						<el-table stripe :data="tableData" border style="width: 100%">
							...
							<el-table-column prop="passage" label="发送内容"> </el-table-column>
			...
			<script src="../js/control7_main.js"></script>
			<script>
			...
			if (request.getAttribute("mlist") == null) {
						request.getRequestDispatcher("../MessageController?t=1").forward(request, response);
					}
				List<MessageInfo> list = (ArrayList) request.getAttribute("mlist");
			//判断空值
				if (list.size() == 0) {
				} else {
				for (MessageInfo v : list) {%>
				app.putMessage(<%=v.getId()%>,'<%=v.getPhone()%>','<%=v.getTime()%>','<%=v.getPassage()%>');
			<%}
						session.setAttribute("addstatus","");
					}%>
